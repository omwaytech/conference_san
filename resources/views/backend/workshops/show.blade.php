                            ...
                            <tbody>
                                @foreach ($workshops as $workshop)
                                    <tr>
                                        ...
                                        <td>{{\Carbon\Carbon::parse($workshop->start_date)->format('d M, Y')}} {{!empty($workshop->end_date) ? ' - '.\Carbon\Carbon::parse($workshop->end_date)->format('d M, Y') : ''}} ({{$workshop->no_of_days}} days)</td>
                                        <td>{{!empty($workshop->time) ? $workshop->time : '-'}}</td>
                                        <td>{{!empty($workshop->registration_deadline) ? \Carbon\Carbon::parse($workshop->registration_deadline)->format('d M, Y') : '-'}}</td>
                                        <td>
                                            ...
                                            @else
                                                @if ($workshop->approved_status == 0)
                                                    @if ($authUser->role == 1)
                                                        <a href="#" class="makeDecision" data-id="{{$workshop->id}}" data-toggle="modal" data-target="#openModal"><span class="badge bg-primary text-white">Pending</span></a>
                                                    @else
                                                ...
                                                @elseif ($workshop->approved_status == 2)
                                                    @if ($authUser->role == 1)
                                                        <a href="#" class="makeDecision" data-id="{{$workshop->id}}" data-toggle="modal" data-target="#openModal"><span class="badge bg-warning">Pending</span></a>
                                                    @else
                                        ...
                                        <td>
                                            <form action="{{route('workshop.destroy', $workshop->id)}}" method="POST">
                                                ...
                                                @if ($authUser->role == 1 && $workshop->is_applied == 0)
                                                    <a href="{{route('workshop.edit', $workshop->id)}}" class="btn btn-sm btn-success mb-1" title="Edit Data"><i class="nav-icon i-Pen-2"></i></a>
                                                @elseif($authUser->role == 2)
                                                    @if ($workshop->approved_status == 0 || $workshop->approved_status == 2)
                                                        <a href="{{route('workshop.edit', $workshop->id)}}" class="btn btn-sm btn-success mb-1" title="Edit Data"><i class="nav-icon i-Pen-2"></i></a>
                                                    @endif
                                                @endif
                                                <button class="btn btn-sm btn-info viewData mb-1" type="button" data-id="{{$workshop->id}}" data-toggle="modal" data-target="#openModal"><i class="nav-icon i-Eye"></i></button>
                                                @if ($authUser->role == 1 && $workshop->is_applied == 0)
                                                ...
                                                @endif
                                                <br>
                                                <a href="{{route('workshop-trainer.index', $workshop->slug)}}" class="btn btn-sm btn-info mb-1">Trainers</a>
                                                <button class="btn btn-sm btn-primary allocatePrice mb-1" type="button" data-id="{{$workshop->id}}" data-toggle="modal" data-target="#openModal">Allocate Price</button>
                                                @if ($workshop->registrations->where('status', 1)->isNotEmpty())
                                                    <a href="{{route('workshop-registration.registrants', $workshop->slug)}}" class="btn btn-sm btn-success mb-1">Workshop Registrations</a>
                                                    <a href="{{route('workshop-registration.excelExport', $workshop->id)}}" class="btn btn-sm btn-primary mb-1"><i class="nav-icon i-File"></i> Export Registrants</a>
                                                @endif
                                                @if ($workshop->discussions->isNotEmpty())
                                                    <a href="{{route('workshop.discussions', $workshop->slug)}}" class="btn btn-sm btn-primary mb-1">Workshop Discussions</a>
                                                @endif
                                            </form>
                                        </td>
                                    </tr>
